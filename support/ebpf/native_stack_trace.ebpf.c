...
// filter_process_age_ns is set during load time.
BPF_RODATA_VAR(u64, filter_process_age_ns, 0)
...
static EBPF_INLINE bool process_is_too_new(u64 ts)
{
  if (!filter_process_age_ns) {
    return false;
  }
  ...
  if (ts >= start_time && ts - start_time < filter_process_age_ns) {
