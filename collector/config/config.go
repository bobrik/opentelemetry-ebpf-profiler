...
type Config struct {
	...
	FilterProcessAge       time.Duration `mapstructure:"filter_process_age"`
...
func (cfg *Config) Validate() error {
	...
	if cfg.FilterProcessAge < 0 {
		return errors.New(
			"invalid argument for filter-process-age. The value " +
				"should be a non-negative duration. 0 disables process age filtering")
	}
