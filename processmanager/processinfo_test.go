...
func TestAppendAnonymousExecutableMappings(t *testing.T) {
	existing := []process.RawMapping{
		{Vaddr: 0x3000, Flags: elf.PF_R | elf.PF_X},
		{Vaddr: 0x5000, Flags: elf.PF_R | elf.PF_X, Path: "/tmp/assembly.dll"},
	}
	mappings := []process.RawMapping{
	...
	iterateMappings := func(callback func(process.RawMapping) bool) (uint32, error) {
	...
	got, err := appendAnonymousExecutableMappings(iterateMappings, existing)
