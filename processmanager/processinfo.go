...
func (pm *ProcessManager) processRemovedInterpreters(pid libpf.PID,
	...
	for key, instance := range pm.interpreters[pid] {
		usesAnonymousMappings := instance.UsesAnonymousMappings()
		if _, ok := interpretersValid[key]; ok {
			anonymousMappingsWanted = anonymousMappingsWanted || usesAnonymousMappings
			continue
		}
...
func appendAnonymousExecutableMappings(iterateMappings func(func(process.RawMapping) bool) (uint32, error),
	interpreterMappings []process.RawMapping,
) ([]process.RawMapping, error) {
	seen := make(map[uint64]libpf.Void, len(interpreterMappings))
	for _, m := range interpreterMappings {
		seen[m.Vaddr] = libpf.Void{}
	}

	_, err := iterateMappings(func(m process.RawMapping) bool {
		if !m.IsExecutable() || !m.IsAnonymous() {
			return true
		}
		if _, ok := seen[m.Vaddr]; ok {
			return true
		}
	...
	return interpreterMappings, err
}
...
func (pm *ProcessManager) SynchronizeProcess(pr process.Process) {
	...
	if intrp, ok := pm.interpreters[pid]; ok {
		...
		for _, instance := range intrp {
			collectAnonymousMappings = collectAnonymousMappings || instance.UsesAnonymousMappings()
		}
	}
	...
	// They are usually in /proc/PID/maps order. Anonymous mappings replayed after
	// late interpreter attachment may be appended out of order.
	...
	skippedAnonymousExecutableMappings := false
	...
	numParseErrors, err := pr.IterateMappings(func(m process.RawMapping) bool {
		...
		anonymousExecutableMapping := m.IsExecutable() && m.IsAnonymous()
		if anonymousExecutableMapping && !collectAnonymousMappings {
			skippedAnonymousExecutableMappings = true
		}

		// Executable mappings and VDSO, converted directly to libpf.FrameMapping
		mappingNeeded := m.IsExecutable() && !m.IsAnonymous()
		// Needed for JIT mappings (Hotspot, V8, BEAM, etc.) after an attached
		// interpreter has announced interest in anonymous executable mappings.
		interpreterNeeded := collectAnonymousMappings && anonymousExecutableMapping
	...
	if skippedAnonymousExecutableMappings && collectAnonymousMappings {
		var replayErr error
		interpreterMappings, replayErr = appendAnonymousExecutableMappings(pr.IterateMappings, interpreterMappings)
		if replayErr != nil {
			log.Debugf("Failed to replay anonymous executable mappings for PID %d: %v", pid, replayErr)
		}
	}
