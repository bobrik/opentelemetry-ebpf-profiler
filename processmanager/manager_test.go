...
func (s *fakeKernelSymbolsSnapshot) BPFGeneration() kallsyms.Generation {
	return s.bpfGeneration
}

func (s *fakeKernelSymbolsSnapshot) KernelGeneration() kallsyms.Generation {
	return s.kernelGeneration
}
...
func TestKernelFrameCacheIgnoresInvalidEntries(t *testing.T) {
	...
	snapshot := &fakeKernelSymbolsSnapshot{
		bpfGeneration: currentGeneration,
		resolutions: map[libpf.Address]kallsyms.AddressResolution{
			address: {
				Source:     kallsyms.SymbolSourceBPF,
