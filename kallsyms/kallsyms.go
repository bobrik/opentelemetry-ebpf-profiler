...
// maxAddr is the max address value.
const maxAddr = uint64(1<<uint(pointerBits) - 1)
...
func nextModuleGeneration(old *moduleTable) uint64 {
	if old == nil {
		return 1
	}
...
func (s *Symbolizer) updateSymbolsFrom(r io.Reader) error {
	...
	for scanner.Scan() {
		...
		// Verify address fits within pointer bounds. Check explicitly to satisfy CodeQL.
		if address > maxAddr {
			return fmt.Errorf("address exceeds pointer size: %x > %x", address, maxAddr)
		}
		if address != 0 {
			noSymbols = false
		}
	...
	s.modules.Store(&moduleTable{
		generation: nextModuleGeneration(oldTable),
...
// Generations returns the BPF and module table generations for this snapshot.
func (s Snapshot) Generations() (bpf, modules uint64) {
	if s.bpf != nil {
		bpf = s.bpf.generation
	}
	if s.modules != nil {
		modules = s.modules.generation
	}
	return bpf, modules
}
