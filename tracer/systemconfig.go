...
func determineSysConfig(coll *cebpf.CollectionSpec, maps map[string]*cebpf.Map,
	kmod *kallsyms.Module, includeTracers types.IncludedTracers, needProcessStartTime bool,
	vars *sysConfigVars,
) error {
	needTPBase := includeTracers.Has(types.PerlTracer) || includeTracers.Has(types.PythonTracer) ||
	...
	log.Infof(
		"Found offsets: task stack %#x, pt_regs %#x, tpbase %#x, group_leader %#x, start_time %#x",
...
func loadRodataVars(coll *cebpf.CollectionSpec, kmod *kallsyms.Module, cfg *Config) error {
	if cfg.FilterProcessAge < 0 {
		return fmt.Errorf("filter process age must be non-negative: %s", cfg.FilterProcessAge)
	}
	...
	if err := coll.Variables["filter_process_age_ns"].Set(uint64(cfg.FilterProcessAge)); err != nil {
		return fmt.Errorf("failed to set filter_process_age_ns: %v", err)
	}
	...
	if err := determineSysConfig(
		systemAnalysisColl, maps, kmod, cfg.IncludeTracers, cfg.FilterProcessAge > 0,
		&rodataVars,
	); err != nil {
